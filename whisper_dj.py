...
def get_audio(video_id):
    for i in range(3):
        try:
            ...
            print(f'Failed to get title for {video_id} on attempt {i+1}... trying again')
            time.sleep(5)
            continue
    yt.streams.filter(only_audio=True).first().download(output_path=f'{BASE_PATH}/data/', filename=f'{vid_id}.mp4')
    subprocess.run(['ffmpeg', '-i', f'{BASE_PATH}/data/{vid_id}.mp4', f'{BASE_PATH}/data/{vid_id}.mp3'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    os.remove(f'{BASE_PATH}/data/{vid_id}.mp4')
    return title,description,vid_id,keywords

def get_transcription(model, vid_id):
    transcription = model.transcribe(f'{BASE_PATH}/data/{vid_id}.mp3', language='de', fp16=False, initial_prompt='Make sentences shorter.')
...
def sentence_splitting(text: str):
    ...
    j = json.loads(response.content.decode(), object_hook=lambda d: SimpleNamespace(**d))
    sentences = []
...
def process_video(model, tag, video_id):
    ...
    text = re.sub(r'(\d)\.', r'\1.\n', transcription['text'])
    data = get_metadata(tag, video_id, title, description, transcription['segments'], keywords)
...
def process_video_list(model, video_list):
    video_list['Tags'] = video_list['Jahrgangsstufe (bei Zielgruppe Schule)'] + ', ' + video_list['Schlagworte'] + ', ' + video_list['Kategorie']
    data = []
    # loop over video_list dataframe
    for _, video in tqdm(video_list.iterrows(), desc='Videos', total=video_list.shape[0]):
        try:
...
if __name__ == '__main__':
    ...
    playlists = ['https://www.youtube.com/playlist?list=PLLTAHuUj-zHiUyJimu33V8xep0VOQYVw0',
                 'https://www.youtube.com/playlist?list=PLLTAHuUj-zHjqKr3k2YwD8m1tzhqlHHr0',
                 'https://www.youtube.com/playlist?list=PLLTAHuUj-zHhk-YUb4y3NGM324XnkyGY-',
